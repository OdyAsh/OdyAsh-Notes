...
1. [Working With Tables (Power BI Model View)](#Working%20With%20Tables%20(Power%20BI%20Model%20View))
	...
	1. [Creating a Date Table](#Creating%20a%20Date%20Table)
		...
		1. [Marking as Official Date Table](#Marking%20as%20Official%20Date%20Table)
...
![Pasted image 20230929210107](../../Media/Default/Pasted%20image%2020230929210107.png)
...
![Pasted image 20230929211103](../../Media/Default/Pasted%20image%2020230929211103.png)
...
* Parameters used in Power BI Reports.
	...
	* Example ([source](https://www.youtube.com/watch?v=1eurc0EY2Xg)):
	  ...
	  ![Pasted image 20231014213104](../../Media/Default/Pasted%20image%2020231014213104.png)
	  Result:
...
![Pasted image 20230930081913](../../Media/Default/Pasted%20image%2020230930081913.png)
...
then:
![Pasted image 20230930112732](../../Media/Default/Pasted%20image%2020230930112732.png)
then:
...
![Pasted image 20230930120218](../../Media/Default/Pasted%20image%2020230930120218.png)
...
Visualized example:
![Pasted image 20230930123719](../../Media/Default/Pasted%20image%2020230930123719.png)
...
(side note: vertipaq is just a case scenario name for an [](.md#^v8cl9b|import%20source%20group)).
...
![Pasted image 20230930141911](../../Media/Default/Pasted%20image%2020230930141911.png)
...
![Pasted image 20231002204342](../../Media/Default/Pasted%20image%2020231002204342.png)
...
![Pasted image 20231002222143](../../Media/Default/Pasted%20image%2020231002222143.png)
...
![Pasted image 20231004100904](../../Media/Default/Pasted%20image%2020231004100904.png)
...
Example 2:
![450](../../Media/Default/Pasted%20image%2020231002225012.png)
...
- You might have lower performance when using bi-directional cross-filtering with many-to-many relationships. This is why each of the three cases above have alternatives:
	- Case 1 alternative: 
		...
		- Convert M:M relationship by using a bridging table (i.e., factless fact table) ([MD source](https://learn.microsoft.com/en-us/power-bi/guidance/star-schema#factless-fact-tables:~:text=A%20more%20compelling,bridging%20table.)). Example ([source](https://learn.microsoft.com/en-us/power-bi/guidance/relationships-many-to-many#relate-many-to-many-dimensions:~:text=Two%20one%2Dto%2Dmany%20relationships%20are%20added%20to%20relate%20the%20tables.%20Here%27s%20an%20updated%20model%20diagram%20of%20the%20related%20tables.%20A%20fact%2Dtype%20table%20named%20Transaction%20has%20been%20added.%20It%20records%20account%20transactions.%20The%20bridging%20table%20and%20all%20ID%20columns%20have%20been%20hidden.)):
		  ![Pasted image 20231018220516](../../Media/Default/Pasted%20image%2020231018220516.png) ^zuytek
	...
	- Case 3 alternative: Use CROSSJOIN DAX function. Example: assuming we have a data model like this:
	  ...
	  ![Pasted image 20231018214507](../../Media/Default/Pasted%20image%2020231018214507.png)
...
![Pasted image 20231006100849](../../Media/Default/Pasted%20image%2020231006100849.png)
...
![Pasted image 20231006161034](../../Media/Default/Pasted%20image%2020231006161034.png)
...
![Pasted image 20231006162041](../../Media/Default/Pasted%20image%2020231006162041.png)
...
![Pasted image 20231006145032](../../Media/Default/Pasted%20image%2020231006145032.png)
...
```
Result:
![Pasted image 20231006153817](../../Media/Default/Pasted%20image%2020231006153817.png)
...
```
Result:
![Pasted image 20231006154002](../../Media/Default/Pasted%20image%2020231006154002.png)
...
![Pasted image 20231005220605](../../Media/Default/Pasted%20image%2020231005220605.png)
...
```
Result:
![Pasted image 20231006181644](../../Media/Default/Pasted%20image%2020231006181644.png)
Notes: 
...
Result:
![Pasted image 20231006182745](../../Media/Default/Pasted%20image%2020231006182745.png)
...
```
Result:
![Pasted image 20231006182825](../../Media/Default/Pasted%20image%2020231006182825.png)
Note:
...
![380](../../Media/Default/Pasted%20image%2020231005220808.png)
...
Then:
![Pasted image 20231007112753](../../Media/Default/Pasted%20image%2020231007112753.png)
...
![Pasted image 20231007112954](../../Media/Default/Pasted%20image%2020231007112954.png)

The log information for each visual shows how much time it took (duration) to complete the following categories of tasks:
![Pasted image 20231007113032](../../Media/Default/Pasted%20image%2020231007113032.png)
	...
	* When using **DirectQuery** storage mode, we can apply one of the following **query reduction options**:
		...
		* *Filters* - By default, the Instantly apply basic filter changes option is selected. To force the report users to manually apply filter changes, select one of the alternative options:
			...
			* Add a single apply button to the filter pane to apply changes at once (preview)
			  ![Pasted image 20231007122635](../../Media/Default/Pasted%20image%2020231007122635.png)
* Regarding visuals:
...
* Regarding DAX
	* Optimize the DAX code by using more efficient functions.
		* Example: [](.md#^a8h0ge|replacing%20FILTER%20with%20KEEPFILTERS%20whenever%20possible). Other examples are found [here](https://medium.com/@technologIT/optimizing-dax-code-for-directquery-tips-and-tricks-ab1b5e6fd91f) and [here](https://towardsdatascience.com/analyze-performance-when-aggregating-data-in-power-bi-and-dax-queries-fc00027950a3).
	* Try to use variables [](.md#^4lu9ab|when%20repeated%20code%20is%20found) to decrease run time.
	...
	* A role can include 0 or more rules.
		...
		* Rules can be static or dynamic
			...
			* Example rule that restricts data access to the region(s) of the authenticated user: `'AppUser'[UserName] = USERPRINCIPALNAME()`. Thiss will compare the current user's UPN with the ones in the `AppUser` table which will map that user to a specific region, as can be seen by the image below:
			  ![Pasted image 20231007170636](../../Media/Default/Pasted%20image%2020231007170636.png)
			* Another example ([M source](https://learn.microsoft.com/en-us/training/modules/row-level-security-power-bi/3-dynamic-method)):
			  ![Pasted image 20231017150459](../../Media/Default/Pasted%20image%2020231017150459.png)
			  then:
			  ![Pasted image 20231017150411](../../Media/Default/Pasted%20image%2020231017150411.png) 
...
* Validate roles to ensure they apply the correct filters.
  ![Pasted image 20231007171243](../../Media/Default/Pasted%20image%2020231007171243.png)
* To add members to the role in Power BI service ([M source](https://learn.microsoft.com/en-us/training/modules/row-level-security-power-bi/2-static-method)):
	...
	* Click here:
	  ![Pasted image 20231017150849](../../Media/Default/Pasted%20image%2020231017150849.png)
...
![Pasted image 20231007171725](../../Media/Default/Pasted%20image%2020231007171725.png)
...
* when a user doesn’t have permission to access a table or column, they get this:
  ![Pasted image 20231007181745](../../Media/Default/Pasted%20image%2020231007181745.png)
...
* We can’t set table-level security if it breaks a relationship chain:
  ![Pasted image 20231007181924](../../Media/Default/Pasted%20image%2020231007181924.png)
...
- Dashboard
	...
	- Dashboards help answer questions such as "How are we doing?" or "Are we there yet?" (recall [](Data%20Analytics%20Notes.md#Types%20of%20Analytics|descriptive%20analysis)).
- Analytical
	- High level of UX to discover answers to a broad array of questions.
	- help answer questions such as "Why did that happen?" or "What might happen next?" (recall [](Data%20Analytics%20Notes.md#Types%20of%20Analytics|diagnostic%20analysis)).
...
![550](../../Media/Default/Pasted%20image%2020231008112343.png)
...
* [](.md#^fupz9h|Visual%20header%20tooltip%20icon)
* [](.md#^0zoeia|Button%20with%20overlay)
...
![Pasted image 20231014115252](../../Media/Default/Pasted%20image%2020231014115252.png)
...
![Pasted image 20231008144830](../../Media/Default/Pasted%20image%2020231008144830.png)
...
* consider design principles of:
	* placement
	  ![425](../../Media/Default/Pasted%20image%2020231008145104.png)
	  ![425](../../Media/Default/Pasted%20image%2020231008145113.png)
	* balance
		...
		* Asymmetrical: achieved through contrast. Example of this is done using the golden ratio:
		  ![425](../../Media/Default/Pasted%20image%2020231008145315.png)
	* contrast
	* proximity
	* repition
...
* Space: Spacing applies to the report page margins and the spacing between report objects.
	* Margins
	  ![Pasted image 20231008145553](../../Media/Default/Pasted%20image%2020231008145553.png)
	* Object spacing: 
...
* Alignment:
	...
	* Consider laying out the report page with different sections and aligning visuals appropriately within the sections. Sections can be _implied_ or _explicit_:
		* Implied sections: Define implied sections by aligning groups of visuals in close proximity:
		  ![Pasted image 20231008145940](../../Media/Default/Pasted%20image%2020231008145940.png)
		* Explicit sections: define explicit sections by using colored shapes and overlaying aligned visuals on those shapes:
		  ![Pasted image 20231008150013](../../Media/Default/Pasted%20image%2020231008150013.png)
...
- **Elements** - Provide visual interest but don't use dataset data.
	- The text box deserves a special mention because it's capable of embedding _dynamic values_ that are sourced from the report dataset into paragraphs of text. When the page is filtered, dynamic values are filtered. Technically, the text box isn't a visual. However, in this instance, it behaves like one. It's also available as the _smart narrative_ visual, which automatically summarizes data by using text descriptions and insights.
	  ![Pasted image 20231008171105](../../Media/Default/Pasted%20image%2020231008171105.png)
...
* Time Series Visuals
	...
	* Line charts work well with a consistent flow of data, such as when sales are recorded for every period. If no sales are recorded for some periods, the line chart visual will fill such gaps with a straight line that connects the values of the previous and next periods.
		* If missing values are a possibility, a column chart might be a better visual choice because it will help to avoid the interpretation of a non-existent trend:
		  ![Pasted image 20231008172013](../../Media/Default/Pasted%20image%2020231008172013.png)
* Proportional visuals
	...
	* In the following example, a 100% Stacked Bar chart visual shows proportional sales across four stores:
	  ![Pasted image 20231008190221](../../Media/Default/Pasted%20image%2020231008190221.png)
	  Notice that the actual sales value isn't shown. Instead, the proportion of sales is shown.
* Numeric visuals
	* Example:
	  ![Pasted image 20231008190500](../../Media/Default/Pasted%20image%2020231008190500.png)
* Grid visuals
	* tables and matrices can effectively convey a lot of detailed information.
		...
		* Matrices can have groups on columns and rows.
			...
			* In the following example, a table visual shows sales and units sold by product. Showing these metrics together in a single visual can be a challenge because ***the scale of values for sales and units is so different***. But by applying conditional formatting, data bars help report consumers quickly ***understand the distribution of values***:
			  ![Pasted image 20231008214400](../../Media/Default/Pasted%20image%2020231008214400.png)
			  ...
			  ![Pasted image 20231008214442](../../Media/Default/Pasted%20image%2020231008214442.png)
...
* Performance visuals
	...
	* Now, to select a KPI:
		* Click this visual:
		  ![Pasted image 20231008215633](../../Media/Default/Pasted%20image%2020231008215633.png)
		* Enter fields for said visual like this:
		  ![Pasted image 20231008215703](../../Media/Default/Pasted%20image%2020231008215703.png)
		* Result will look something like this (changes based on value of `Trend axis`):
		  ![Pasted image 20231008215718](../../Media/Default/Pasted%20image%2020231008215718.png)
* AI visuals
...
* Illustration of visual headers:
  ![Pasted image 20231014113536](../../Media/Default/Pasted%20image%2020231014113536.png)
...
* Regarding visual header tooltip icons ([M source](https://learn.microsoft.com/en-us/training/modules/power-bi-effective-user-experience/7-design-built-in-assistance#:~:text=or%20Help%20icon.-,Visual%20header%20tooltip%20icon,-Within%20the%20visual)) ^fupz9h
	* we can enable the visual header tooltip. This adds the **Help** (**?**) icon to the visual header. Illustration:
	  ![Pasted image 20231014114158](../../Media/Default/Pasted%20image%2020231014114158.png)
...
- Report, page, and visual level filters apply to the structure of the report:
  ![Pasted image 20231011072601](../../Media/Default/Pasted%20image%2020231011072601.png)
...
![202](../../Media/Default/Pasted%20image%2020231011072831.png)
...
* You can hide filters. A hidden filter isn't visible to report consumers:
  ![Pasted image 20231011073529](../../Media/Default/Pasted%20image%2020231011073529.png)
* You can hide the entire **Filters** pane to ensure that report consumers can't open it:
  ![Pasted image 20231011073553](../../Media/Default/Pasted%20image%2020231011073553.png)
...
* Dropdown
	...
	* <mark style="background: #D2B3FFA6;">only query the dataset when expanded open. Therefore, they can also help expedite report page rendering.</mark>
		* Note: other slicer types can have this advantage as well based on [](.md#^e1vpye|how%20you%20configure%20Power%20BI%20settings%20to%20gain%20performance%20boost).
...
![Pasted image 20231011075824](../../Media/Default/Pasted%20image%2020231011075824.png)
...
* Example of disabling cross filters for a certain visual (from video found [here](https://learn.microsoft.com/en-us/training/modules/power-bi-effective-filters/4-advanced-filtering#:~:text=To%20gain%20a%20better%20understanding%20of%20visual%20interactions%2C%20watch%20the%20following%20video%20that%20describes%20a%20use%20case%20and%20shows%20how%20to%20configure%20it.)):
  ![Pasted image 20231011092414](../../Media/Default/Pasted%20image%2020231011092414.png)
  To find filters currently a visual:
  ![Pasted image 20231011092518](../../Media/Default/Pasted%20image%2020231011092518.png)
...
![Pasted image 20231011092806](../../Media/Default/Pasted%20image%2020231011092806.png)
...
* on that created page, drag the field for which you want to enable drillthrough here:
  ![Pasted image 20231011093048](../../Media/Default/Pasted%20image%2020231011093048.png)
...
![Pasted image 20231011104446](../../Media/Default/Pasted%20image%2020231011104446.png)
...
![Pasted image 20231011104607](../../Media/Default/Pasted%20image%2020231011104607.png)
...
* One way to use tooltips is to display ***graphical information***. To do so, follow the steps [here](https://learn.microsoft.com/en-us/training/modules/power-bi-effective-reports/7-format#:~:text=Another%20way%20to,tooltip%20will%20display.).
	* Note: the final output could look like this (when hovering over the doughnut visual):
	  ![Pasted image 20231008215346](../../Media/Default/Pasted%20image%2020231008215346.png)
...
- Illustration on how to create a bookmark ([M source](https://learn.microsoft.com/en-us/training/modules/power-bi-effective-user-experience/4-bookmarks#:~:text=watch%20the%20following%20video.)):
  ![Pasted image 20231014110924](../../Media/Default/Pasted%20image%2020231014110924.png)
...
- Bookmark examples ([M source](https://learn.microsoft.com/en-us/training/modules/power-bi-effective-user-experience/4-bookmarks#:~:text=the%20following%20video.-,Bookmark%20examples,-By%20combining%20different)):
	...
	- Pop-up overlays. Illustration:
	  ![Pasted image 20231014112341](../../Media/Default/Pasted%20image%2020231014112341.png)
...
![Pasted image 20231011125447](../../Media/Default/Pasted%20image%2020231011125447.png)
...
![Pasted image 20231011135812](../../Media/Default/Pasted%20image%2020231011135812.png)
...
![Pasted image 20231011141228](../../Media/Default/Pasted%20image%2020231011141228.png)
...
* To create statistical functions, you have the following options:
	* create **quick statistics** from the Visualizations pane:
	  ![Pasted image 20231014134042](../../Media/Default/Pasted%20image%2020231014134042.png)
...
* Using histograms is the most common way to display statistics about your datasets.
	* Histograms are created using the **bar (column) chart** or the **area chart** visuals:
	  ![Pasted image 20231014134529](../../Media/Default/Pasted%20image%2020231014134529.png)
...
* Performing Top N analysis is a great way to present data that might be important, such as the top 10 selling products. This can be done in 3 ways:
	* Q&A Visual
	  ![Pasted image 20231014134808](../../Media/Default/Pasted%20image%2020231014134808.png)
	  ![Pasted image 20231014134811](../../Media/Default/Pasted%20image%2020231014134811.png)
	  Side note: Q&A is explained later in the [Q&A section](#Q&A). ^uhio1l
	* Top N filter type
	  ![Pasted image 20231014134858](../../Media/Default/Pasted%20image%2020231014134858.png)
	* TOPN DAX function
...
* The best visual to use for identifying outliers is the scatter chart;
	...
	* Basic way of identifying outliers:
		...
		* add fields of interests:
		  ![Pasted image 20231014135443](../../Media/Default/Pasted%20image%2020231014135443.png)
		* Identify the outliers by yourself:
		  ![Pasted image 20231014135503](../../Media/Default/Pasted%20image%2020231014135503.png)
	* Advanced way of identifying outliers (on a different example than the one above) ([source](https://www.youtube.com/watch?v=kc3ztxkj0Tc)):
		* Suppose you want to see the outliers here:
		  ![Pasted image 20231014140104](../../Media/Default/Pasted%20image%2020231014140104.png)
		...
		* Therefore, if we want to manually see where the outliers are, we'll have to look at the x and y axes to see them:
		  ![Pasted image 20231014140513](../../Media/Default/Pasted%20image%2020231014140513.png)
		* But what if we want to automate this process by separating the y axis (i.e., `Total Sales`) into two groups (i.e., outliers, and non outliers)? Illustration:
		  ![Pasted image 20231014140700](../../Media/Default/Pasted%20image%2020231014140700.png)
		* Solution: follow these steps: 
			* Create a *detached table* called `Outlier Detection Logic` (at minute 3:50, 5:00):
			  ![Pasted image 20231014135610](../../Media/Default/Pasted%20image%2020231014135610.png)
			  ![Pasted image 20231014135709](../../Media/Default/Pasted%20image%2020231014135709.png)
		* create DAX measure called `Outlier Sales` that will follow the business logic discussed [](.md#^kmz928|above) in order to sum the `Total Sales` of the outlier customers only (explained at [6:00](https://youtu.be/kc3ztxkj0Tc?t=362)):
		  ![Pasted image 20231014141124](../../Media/Default/Pasted%20image%2020231014141124.png)
		* create complimentary logic for `Non Outlier Sales` measure:
		  ![Pasted image 20231014142039](../../Media/Default/Pasted%20image%2020231014142039.png)
		* As you can see, these are two DAX formulas. However, we need to use only one DAX formula called `Sales Grouping` to display in the y-axis of the scatter chart. To do so, we use this DAX formula (explained at [8:25](https://youtu.be/kc3ztxkj0Tc?t=503)):
		  ![Pasted image 20231014143127](../../Media/Default/Pasted%20image%2020231014143127.png)
		* Finally, we add the created measures as fields to the scatter chart:
		  ![Pasted image 20231014144229](../../Media/Default/Pasted%20image%2020231014144229.png)
		  ...
		  ![Pasted image 20231014144750](../../Media/Default/Pasted%20image%2020231014144750.png)
...
* Prepare your visual. For example:
  ![Pasted image 20231014145625](../../Media/Default/Pasted%20image%2020231014145625.png)
* Add the <mark style="background: #FF5582A6;">group field</mark> to the `Axis` well like so:
  ![Pasted image 20231014145456](../../Media/Default/Pasted%20image%2020231014145456.png)
* You'll now see the visual properly grouped/binned on the chosen axis like so:
  ![Pasted image 20231014145542](../../Media/Default/Pasted%20image%2020231014145542.png)

Now, regarding the ***group field***, it can be created as a:
* Measure (using DAX)
	* Create measure using this DAX expression (at minute 1:07)
	  ![Pasted image 20231014150002](../../Media/Default/Pasted%20image%2020231014150002.png)
* Group (using Power BI's UI) (at 2:02)
	* click on any of these two options:
	  ![Pasted image 20231014150205](../../Media/Default/Pasted%20image%2020231014150205.png)
	* now, in the new window that opens, you have 2 group types to choose from:
		* List (at 2:21):
		  ![Pasted image 20231014150331](../../Media/Default/Pasted%20image%2020231014150331.png)
		*  Bins (screenshot is from another [video](https://youtu.be/BRvdZSfO0DY?t=257)):
		  ![Pasted image 20231014150805](../../Media/Default/Pasted%20image%2020231014150805.png)
* Column (using Power Query) (at [3:12](https://youtu.be/D1LO8syugMA?t=192))
	...
	* Highlight the column that you want to bin, then click "Column from examples":
	  ![Pasted image 20231014151033](../../Media/Default/Pasted%20image%2020231014151033.png)
	* At the first row, type a bin size like "20-40":
	  ![Pasted image 20231014151111](../../Media/Default/Pasted%20image%2020231014151111.png)
	* Result:
	  ![Pasted image 20231014151129](../../Media/Default/Pasted%20image%2020231014151129.png)
...
![Pasted image 20231014152650](../../Media/Default/Pasted%20image%2020231014152650.png)
...
* We can also import a time series custom visual into Power BI Desktop from Microsoft AppSource. Example ([M source](https://learn.microsoft.com/en-us/training/modules/perform-analytics-power-bi/6-time-series-analysis#:~:text=When%20you%20have%20set%20up%20the%20Play%20Axis%20visual%20to%20meet%20your%20requirements%2C%20you%20are%20ready%20to%20use%20it%20with%20your%20other%20visuals.%20Select%20the%20Play%20button%20and%20then%20watch%20how%20the%20data%20in%20each%20visual%20on%20the%20page%20evolves%20over%20the%20time.%20You%20can%20use%20the%20control%20buttons%20to%20pause%20the%20animation%2C%20restart%20it%2C%20and%20so%20on.)):
	  ![line chart auto play](../../Media/Default/line%20chart%20auto%20play.gif)
...
* Example:
  ![Pasted image 20231014161057](../../Media/Default/Pasted%20image%2020231014161057.png)
  ![Pasted image 20231014161105](../../Media/Default/Pasted%20image%2020231014161105.png)
...
* Example ([M source](https://learn.microsoft.com/en-us/training/modules/perform-analytics-power-bi/9-use-specialized-visuals#:~:text=The%20capabilities%20of%20the%20Key%20influencers%20visual%20are%20best%20described%20by%20following%20an%20example.%20For%20a%20demonstration%20of%20the%20Key%20influencers%20visual%2C%20watch%20the%20following%20video.)): 
  ![Pasted image 20231014172359](../../Media/Default/Pasted%20image%2020231014172359.png)
...
* Example ([M source](https://learn.microsoft.com/en-us/training/modules/perform-analytics-power-bi/9-use-specialized-visuals#:~:text=The%20capabilities%20of%20the%20Decomposition%20Tree%20visual%20are%20best%20described%20by%20following%20an%20example.%20For%20a%20demonstration%20of%20the%20Decomposition%20Tree%20visual%2C%20watch%20the%20following%20video.)):
  ![Pasted image 20231014172809](../../Media/Default/Pasted%20image%2020231014172809.png)
...
Side note: Q&A can be used in top N analysis as mentioned in the [](.md#^uhio1l|Statistical%20Summaries%20section)
...
* To optimize the Q&A experience:
	...
	* These optimizations can be done by clicking at the cog icon of the Q&A visual ([M source](https://learn.microsoft.com/en-us/training/modules/perform-analytics-power-bi/9-use-specialized-visuals#:~:text=For%20a%20demonstration%20of%20the%20Q%26A%20visual%2C%20watch%20the%20following%20video.)):
	  ![Pasted image 20231014173326](../../Media/Default/Pasted%20image%2020231014173326.png)
...
Illustrative example:
  ![Pasted image 20231014174719](../../Media/Default/Pasted%20image%2020231014174719.png)
...
* Then:
  ![Pasted image 20231014175857](../../Media/Default/Pasted%20image%2020231014175857.png)
* Then:
  ![Pasted image 20231014175930](../../Media/Default/Pasted%20image%2020231014175930.png)
* Regarding (3), the **Connection Properties** screen will appear, so follow these steps:
  ![Pasted image 20231014180013](../../Media/Default/Pasted%20image%2020231014180013.png)
...
* Follow these steps:
  ![Pasted image 20231014180346](../../Media/Default/Pasted%20image%2020231014180346.png)
...
* Follow the video [here](https://youtu.be/hEGDJ6SxJEk?t=461) (starting from 7:40) to see the steps of creating a matrix:
  ![Pasted image 20231014211345](../../Media/Default/Pasted%20image%2020231014211345.png)
...
* Or follow the steps in the same video referenced above starting from [10:00](https://youtu.be/hEGDJ6SxJEk?t=604):
  ![Pasted image 20231014211141](../../Media/Default/Pasted%20image%2020231014211141.png)
  when clicking "Run":
  ![Pasted image 20231014211221](../../Media/Default/Pasted%20image%2020231014211221.png)
...
* First method of adding charts:
  ![Pasted image 20231014211618](../../Media/Default/Pasted%20image%2020231014211618.png)
* Second method of adding charts:
  ![Pasted image 20231014211625](../../Media/Default/Pasted%20image%2020231014211625.png)
...
![Pasted image 20231014211720](../../Media/Default/Pasted%20image%2020231014211720.png)
...
* **Admin**
    ...
    - Change [](.md#^u7k3wc|data%20classifications) that could be applied to a dashboard.
- **Member**
...
- Note:  full permissions, review the [Roles in workspaces documentation](https://learn.microsoft.com/en-us/power-bi/collaborate-share/service-roles-new-workspaces#workspace-roles). It is mentioned here also:
  ![Pasted image 20231015072419](../../Media/Default/Pasted%20image%2020231015072419.png)
...
![Pasted image 20231015072458](../../Media/Default/Pasted%20image%2020231015072458.png)
...
![Pasted image 20231015080346](../../Media/Default/Pasted%20image%2020231015080346.png)
...
* click here:
  ![Pasted image 20231015140500](../../Media/Default/Pasted%20image%2020231015140500.png)
* After creating the pipeline, you'll see this page:
  ![Pasted image 20231015140528](../../Media/Default/Pasted%20image%2020231015140528.png)
...
* After finishing the development workspace, Select **Deploy to test**, which will create a new workspace. 
	...
	* Testing should emulate conditions that objects will experience after they've been deployed for users. 
		* Therefore, Power BI allows you to change the source of data that is used during testing by clicking here:
		  ![Pasted image 20231015140908](../../Media/Default/Pasted%20image%2020231015140908.png)
		  In this example, <mark style="background: #FFB8EBA6;">we want our dataset to be used for testing but with a different data source. To do so we have the following 2 options</mark>:
			...
			* Add a new rule:
			  ![Pasted image 20231015141111](../../Media/Default/Pasted%20image%2020231015141111.png)
			  Then:
			  ![Pasted image 20231015141133](../../Media/Default/Pasted%20image%2020231015141133.png)
...
* Suppose we receive a notification that one of the other developers has modified a report. To see the changes to this report:
  ![Pasted image 20231015141555](../../Media/Default/Pasted%20image%2020231015141555.png)
  Result:
  ![Pasted image 20231015141612](../../Media/Default/Pasted%20image%2020231015141612.png)
...
* Refers to the path that data takes from the data source to the destination.
	* Example:
	  ![Pasted image 20231015143231](../../Media/Default/Pasted%20image%2020231015143231.png) ^zfkx72
...
* Go to a workspace, then:
  ![Pasted image 20231015143628](../../Media/Default/Pasted%20image%2020231015143628.png)
  This will show the graph that is shown [](.md#^zfkx72|above).

Now, regarding each card in the lineage view:
* Data source card
	* Example:
	  ![Pasted image 20231015143854](../../Media/Default/Pasted%20image%2020231015143854.png)
	...
	* Selecting the lower-right icon on the card will highlight the path from the data source to the destination:
	  ![Pasted image 20231015144005](../../Media/Default/Pasted%20image%2020231015144005.png)


Dataset/Dataflow cards:
* Example:
  ![Pasted image 20231015144348](../../Media/Default/Pasted%20image%2020231015144348.png)
* The impact analysis window displays how many workspaces, reports, and dashboards that this dataset is a part of and how many views that this dataset has gathered. We can select **Notify contacts**, which allows you to notify dataset owners (or any other user) of changes in the dataset:
  ![Pasted image 20231015144514](../../Media/Default/Pasted%20image%2020231015144514.png)
* When double clicking this card, we see the following metadata:
  ![Pasted image 20231015144251](../../Media/Default/Pasted%20image%2020231015144251.png)
...
* When double clicking:
  ![Pasted image 20231015144649](../../Media/Default/Pasted%20image%2020231015144649.png)
* when clicking on the ellipsis (...) icon:
  ![Pasted image 20231015144706](../../Media/Default/Pasted%20image%2020231015144706.png)
...
* To enable it:
  ![Pasted image 20231016213638](../../Media/Default/Pasted%20image%2020231016213638.png)
  then:
  ![Pasted image 20231016213647](../../Media/Default/Pasted%20image%2020231016213647.png)
* Possible labels:
	* None
	* Personal
	* General
...
* When exporting a labeled dataset:
	* it could appear like this if we're an authorized user:
	  ![Pasted image 20231016213819](../../Media/Default/Pasted%20image%2020231016213819.png)
...
* flow from a user in the cloud, to the on-premise data, then back to the cloud:
  ![Pasted image 20231016214604](../../Media/Default/Pasted%20image%2020231016214604.png)
	* Behind the scenes, the following actions occur:
	  ![Pasted image 20231016215420](../../Media/Default/Pasted%20image%2020231016215420.png)
...
* select the **Schedule refresh** icon:
  ![Pasted image 20231016220639](../../Media/Default/Pasted%20image%2020231016220639.png)
	* Side note: the on-demand refresh doesn't affect the next scheduled refresh time.
* Do the following:
  ![Pasted image 20231016220543](../../Media/Default/Pasted%20image%2020231016220543.png)
...
* Check it from here:
  ![Pasted image 20231016221038](../../Media/Default/Pasted%20image%2020231016221038.png)
* Or from the refresh history page of the dataset's settings window:
  ![Pasted image 20231016221105](../../Media/Default/Pasted%20image%2020231016221105.png)
...
* Define the filter parameters.
	...
	* Definition steps:
	  ![Pasted image 20231016222315](../../Media/Default/Pasted%20image%2020231016222315.png)
* Use the parameters to apply a filter.
	* Steps:
		* Right click a chosen column, then select **custom filter**:
		  ![Pasted image 20231016222446](../../Media/Default/Pasted%20image%2020231016222446.png)
		* Then, in the **Filter Rows** window that displays:
		  ![Pasted image 20231016222531](../../Media/Default/Pasted%20image%2020231016222531.png)
...
* Right click the chosen table which you've applied the filter for, then:
  ![Pasted image 20231016222655](../../Media/Default/Pasted%20image%2020231016222655.png)
* Then, configure the refresh as required. In this example, you will define a refresh policy to store data for five full calendar years, plus data for the current year up to the current date, and incrementally refresh 10 days of data:
  ![Pasted image 20231016222734](../../Media/Default/Pasted%20image%2020231016222734.png)
...
* This is done by endorsing those optimized datasets as the ***one source of truth***. Endorsement can be done using promotion or certification:
	* Promotion
		...
		* Requirements:
			* write permissions on the workspace containing the content to be promoted. I.e., an [](.md#^68di4d|Admin).
		* Steps:
			* In Power BI Service:
			  ![Pasted image 20231017073316](../../Media/Default/Pasted%20image%2020231017073316.png)
			* Then:
			  ![Pasted image 20231017073336](../../Media/Default/Pasted%20image%2020231017073336.png)
			* Result:
			  ![Pasted image 20231017073421](../../Media/Default/Pasted%20image%2020231017073421.png)
	...
	* Certification
		...
		* Steps:
			* To request certification:
			  ![Pasted image 20231017073558](../../Media/Default/Pasted%20image%2020231017073558.png)
...
* Steps:
	* Click here:
	  ![Pasted image 20231017075030](../../Media/Default/Pasted%20image%2020231017075030.png)
	* ThenSelect the **Datasets** tab and expand the **Query Caching** options to see this:
	  ![Pasted image 20231017075100](../../Media/Default/Pasted%20image%2020231017075100.png)
...
* Its tiles can be sourced from a multitude of places including reports, datasets, other dashboards, Microsoft Excel, SQL Server Reporting Services, and more.
	* A report element pinned in the dashboard acts as a direct connection between the dashboard and the report that the snapshot came from.
		* To pin a report to a dashboard, go to the report visual that you want to pin, then:
		  ![Pasted image 20231017110035](../../Media/Default/Pasted%20image%2020231017110035.png)
		  then:
		  ![Pasted image 20231017110044](../../Media/Default/Pasted%20image%2020231017110044.png)
	* Dashboard tiles can also be created from ***custom streaming data*** like so ([M source](https://learn.microsoft.com/en-us/training/modules/create-dashboards-power-bi/7-configure-real-time-dashboard)):
	  ![Pasted image 20231017133847](../../Media/Default/Pasted%20image%2020231017133847.png)
	  ...
	  ![Pasted image 20231017134036](../../Media/Default/Pasted%20image%2020231017134036.png)
...
* Can create data alerts for pinned visuals:
	* Steps:
	  ![Pasted image 20231017123245](../../Media/Default/Pasted%20image%2020231017123245.png)
	  Then:
	  ![Pasted image 20231017123254](../../Media/Default/Pasted%20image%2020231017123254.png)
...
* Can get quick insights on the dataset
	* Steps:
	  ![Pasted image 20231017132825](../../Media/Default/Pasted%20image%2020231017132825.png)
	  Then, this appears:
	  ![Pasted image 20231017132838](../../Media/Default/Pasted%20image%2020231017132838.png)
	  ...
	  ![Pasted image 20231017132934](../../Media/Default/Pasted%20image%2020231017132934.png)
...
* Can have different dashboard themes
	* Steps:
	  ![Pasted image 20231017133236](../../Media/Default/Pasted%20image%2020231017133236.png)
	  where "Custom" will give us these options:
	  ![Pasted image 20231017133326](../../Media/Default/Pasted%20image%2020231017133326.png)
* Can perform data classification ([M source](https://learn.microsoft.com/en-us/training/modules/create-dashboards-power-bi/8-configure-data-classification))
	...
	* Steps:
	  ![Pasted image 20231017134847](../../Media/Default/Pasted%20image%2020231017134847.png)
	  then:
	  ![Pasted image 20231017134907](../../Media/Default/Pasted%20image%2020231017134907.png)
	  Result:
	  ![Pasted image 20231017135034](../../Media/Default/Pasted%20image%2020231017135034.png)
